/* Test and timing harness program for developing a multichannel
   ...
   Version 1.8 : Bug fixes, modified test of accuracy to take absolute
                 value of array contents into account
                  ...
                  int dim0, int dim1, int dim2)
{
  ...
  if ( sum_abs_diff > EPSILON * sum_abs ) {
    fprintf(stderr, "WARNING: sum of absolute differences (%f) > (EPSILON * sum_abs) (%f)\n",
...
/* the slow but correct version of convolution */
...
int main(int argc, char ** argv)
{
  ...
  /* perform student's multichannel convolution using pthreads */
  student_conv_pthreads(image, kernels, output, width,
                    height, nchannels, nkernels, kernel_order);
  ...
  /* record starting time of student's OpenMP code*/
  gettimeofday(&start_time, NULL);

  /* perform student's multichannel convolution using pthreads */
  student_conv_openmp(image, kernels, output, width,
                    height, nchannels, nkernels, kernel_order);
